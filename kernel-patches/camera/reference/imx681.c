...
#define IMX681_ANA_GAIN_MAX		960	/* inverted in set_ctrl: hw=960-logical */
...
#define IMX681_WIDTH			1920
#define IMX681_HEIGHT			1136
...
/* SP10: Link freq = 19.2MHz × 119 / 3 / 2 = 380.8 MHz */
#define IMX681_LINK_FREQ    		380800000LL

/* PLL multiplier = 119 = 0x77 */
/* PLL2: pre_div=3, multiplier=119 (0x0077) */
...
static const struct cci_reg_sequence imx681_init_regs[] = {
	...
	{ CCI_REG8(0x0111),  0x02 }, /* CSI-2 D-PHY signalling mode */
	/* CSI data format: RAW10 */
	{ CCI_REG16(0x0112), 0x0A0A },
	/* CSI lane mode: 2 lanes */
	...
	/* Frame length = 3177 — Sony vendor register */
	{ CCI_REG16(0x033E), 0x0C69 },
	/* Frame length = 3177 — CCS-standard register (was 0, sensor may use this for MIPI timing) */
	/* Crop window start: X_ADD_STA=100, Y_ADD_STA=256 */
	{ CCI_REG24(0x0345), 0x640178 },
	/* Crop window end: X_ADD_END=3943, Y_ADD_END=2895 */
	{ CCI_REG24(0x0349), 0x670A57 }, /* Crop window end: X_ADD_END=3943, Y_ADD_END=2895 */
	/* Digital crop / vendor config */
	/* Digital crop: explicit write of all 8 registers to known-good values */
	{ CCI_REG8(0x0408),  0x03 }, /* DIG_CROP_X_OFFSET high = 0x03 */
	{ CCI_REG8(0x0409),  0xC2 }, /* DIG_CROP_X_OFFSET low = 0xC2 (962 center) */
	{ CCI_REG8(0x040A),  0x00 }, /* DIG_CROP_Y_OFFSET high = 0 */
	{ CCI_REG8(0x040B),  0x00 }, /* DIG_CROP_Y_OFFSET low  = 0 */
	{ CCI_REG8(0x040C),  0x07 }, /* DIG_CROP_WIDTH high = 0x07 */
	{ CCI_REG8(0x040D),  0x80 }, /* DIG_CROP_WIDTH low = 0x80 (1920) */
	{ CCI_REG8(0x040E),  0x04 }, /* DIG_CROP_HEIGHT high = 0x04 */
	{ CCI_REG8(0x040F),  0x70 }, /* DIG_CROP_HEIGHT low = 0x70 (1136) */
	/* 2x2 binning */
	{ CCI_REG8(0x0900),  0x01 }, /* BINNING_MODE = 1 (enable) */
	{ CCI_REG8(0x0901),  0x22 }, /* BINNING_TYPE = 0x22 (2x2) */
	{ CCI_REG8(0x0401),  0x01 }, /* SCALE_MODE = 1 (horizontal scaler enabled) */
	{ CCI_REG16(0x0404), 0x0020 }, /* SCALE_M = 32 (0.5x horizontal) */
	{ CCI_REG8(0x0902),  0x08 }, /* BINNING_WEIGHTING */
	/* X_OUTPUT_SIZE=3844, Y_OUTPUT_SIZE=2640 */
	{ CCI_REG32(0x034C), 0x07800470 }, /* X_OUTPUT_SIZE=1920, Y=1136 */
	/* PLL multiplier = 225 */
	{ CCI_REG8(0x0307),  0x77 },
	/* PLL2: pre_div=3, multiplier=303 (0x012F) */
	{ CCI_REG24(0x030D), 0x030077 },
...
static const struct cci_reg_sequence imx681_first_exposure[] = {
	...
	{ CCI_REG24(0x0229), 0x000C61 },	/* Exposure = 3169 lines */
	{ CCI_REG16(0x0204), 0x0000 },		/* Analog gain = 0 (hw code 0 = 16x brightest) */
	{ CCI_REG16(0x020E), 0x0400 },		/* Digital gain = 4.0x */
...
static int imx681_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_ANALOGUE_GAIN: {
		 ...
		 */
		/* Hardware register is inverted: code 0=16x, code 960=1x.
		 * Invert so logical 0=1x, logical 960=16x (AGC ramps correctly). */
		int ana_code = 960 - min(ctrl->val, 960);
...
static int imx681_start_streaming(struct imx681 *imx681)
{
	...
	/* CCS FLL must be written separately after streaming when v_flip is active */
	cci_write(imx681->cci, CCI_REG16(0x0340), 3177, NULL);
	msleep(50);
	return 0;
}
...
static int imx681_identify_module(struct imx681 *imx681)
{
	...
	for (tries = 0; tries < 5; tries++) {
		...
		if (ret) {
			dev_warn(imx681->dev,
				 "chip ID read failed (attempt %d/5): %d\n",
				 tries + 1, ret);
			msleep(100);
			continue;
		}
		...
		dev_warn(imx681->dev,
			 "chip ID 0x%04llx != 0x%04x (attempt %d/5), retrying\n",
			 val, IMX681_CHIP_ID, tries + 1);
		msleep(100);
	}

	/* All retries exhausted */
	if (ret) {
		dev_err(imx681->dev, "chip ID read failed after 5 attempts: %d\n", ret);
		return ret;
	}
	dev_err(imx681->dev, "chip ID mismatch after 5 attempts: 0x%04llx != 0x%04x\n",
		val, IMX681_CHIP_ID);
	return -EPROBE_DEFER;
}
